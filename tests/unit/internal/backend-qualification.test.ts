...
describe('provider gaps this library compensates for', () => {
  test('accepts an RSA private JWK whose CRT parameters are inconsistent', () => {
    ...
    expect(() => createPrivateKey({ key: { ...jwk, qi: b64u(qi) }, format: 'jwk' })).not.toThrow();
  });
  ...
  test('echoes supplied EC coordinates instead of deriving them from d', () => {
    ...
    expect(exported.x).toBe(b['x']!);
    expect(exported.x).not.toBe(a['x']!);
  });
  ...
  test('accepts an X25519 private JWK whose public component does not match d', () => {
    ...
    expect(() => createPrivateKey({ key: { ...a, x: b['x']! }, format: 'jwk' })).not.toThrow();
  });
});
...
describe('provider capabilities this library relies on', () => {
  ...
  test('derives EC public points from the private scalar alone', () => {
    for (const curve of EC_CURVES) {
      ...
      if (derived.ok) {
        expect(b64u(derived.value.x)).toBe(jwk['x']!);
        expect(b64u(derived.value.y)).toBe(jwk['y']!);
      }
    }
  });
  ...
  test('derivation ignores any supplied coordinates, detecting a mismatch', () => {
    ...
    expect(derived.ok).toBe(true);
    if (derived.ok) {
      expect(b64u(derived.value.x)).not.toBe(b['x']!);
    }
  });

  test('rejects an out-of-range EC private scalar', () => {
    expect(deriveEcPublicPoint('P-256', new Uint8Array(32)).ok).toBe(false);
  });
  ...
  test('derives OKP public keys from private material', () => {
    for (const curve of OKP_CURVES) {
      ...
      expect(derived.ok).toBe(true);
      if (derived.ok) {
        expect(b64u(derived.value)).toBe(jwk['x']!);
      }
    }
  });
  ...
  test('supports an explicit PSS salt length and rejects a wrong one', () => {
    ...
    expect(verify('sha256', Buffer.from('m'), options, signature)).toBe(true);
    ...
    expect(verify('sha256', Buffer.from('m'), { ...options, saltLength: 48 }, signature)).toBe(false);
  });
});

describe('runtime curve inventory', () => {
  test('provides every currently qualified required curve', () => {
    expect(EC_CURVES).toContain('P-256');
  });
});
...
describe('randomness', () => {
  test('returns the requested length and varies between calls', () => {
    ...
    if (first.ok && second.ok) {
      expect(first.value).toHaveLength(32);
      expect(Buffer.from(first.value).equals(Buffer.from(second.value))).toBe(false);
    }
  });

  test('accepts a zero-length request and rejects invalid lengths', () => {
    expect(systemRandom.randomBytes(0).ok).toBe(true);
    expect(systemRandom.randomBytes(-1).ok).toBe(false);
    expect(systemRandom.randomBytes(1.5).ok).toBe(false);
  });
});
...
describe('constant-time comparison', () => {
  test('compares equal and unequal values of the same length', () => {
    ...
    expect(constantTime.equal(a, new Uint8Array([1, 2, 3, 4]))).toBe(true);
    expect(constantTime.equal(a, new Uint8Array([1, 2, 3, 5]))).toBe(false);
    // Differing in the first byte must behave the same as differing in the last.
    expect(constantTime.equal(a, new Uint8Array([9, 2, 3, 4]))).toBe(false);
  });

  test('returns false for different lengths instead of throwing', () => {
    expect(constantTime.equal(new Uint8Array([1]), new Uint8Array([1, 2]))).toBe(false);
    expect(constantTime.equal(new Uint8Array(), new Uint8Array([1]))).toBe(false);
    expect(constantTime.equal(new Uint8Array(), new Uint8Array())).toBe(true);
  });
});
...
describe('WebCrypto backend selection', () => {
  test('rejects the mismatched EC private key the native module accepts', async () => {
    ...
    expect(() => createPrivateKey({ key: mismatched, format: 'jwk' })).not.toThrow();
  ...
  test('accepts an undersized HMAC key, so the length bound is enforced here', () => {
    // The provider imposes no minimum, which is why `computeHmac` checks the
    // key against the hash output size before signing.
    expect(
      crypto.subtle.importKey('raw', new Uint8Array(8), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']),
